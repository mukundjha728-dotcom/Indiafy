    ...
    <div className="os-hero relative flex flex-col items-center text-center pt-10 pb-8 px-4">
      ...
      <div className="mt-5 flex items-center gap-2 bg-white border border-stone-200 rounded-full px-5 py-2.5 shadow-sm">
        ...
        <button className="text-emerald-600 hover:text-emerald-700 transition-colors ml-1">
          <Icon d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2 2v8a2 2 0 002 2z" className="w-3.5 h-3.5" />
        </button>
      </div>
    </div>
  );
      ...
      <div className="mt-4 pt-4 border-t border-stone-100 space-y-2">
        {[
          ...
          <div key={r.l} className="flex items-center justify-between">
            ...
            <span className={`text-sm font-bold ${r.green ? "text-emerald-600" : r.bold ? "text-stone-900 text-base" : "text-stone-700"}`}>
              {r.v}
            </span>
          </div>
        ))}
...
function ActionButtons() {
  const navigate = useNavigate(); // 2. Add navigation hook here
  ...
  return (
    <div className="os-card-5 flex flex-col sm:flex-row gap-3">
      ...
      <button 
        ...
        className="btn-primary flex-1 flex items-center justify-center gap-2.5 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-sm rounded-2xl shadow-lg shadow-emerald-200 active:scale-[.98] transition-all"
      >
      ...
      <button 
        onClick={() => navigate('/')}
        className="flex-1 flex items-center justify-center gap-2.5 py-4 bg-white hover:bg-stone-50 text-stone-800 font-bold text-sm rounded-2xl border-2 border-stone-200 hover:border-stone-300 active:scale-[.98] transition-all shadow-sm"
      >
...
function RecommendedProducts() {
  const navigate = useNavigate(); // 5. Added navigation here as well
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {RECOMMENDED.map((p, i) => {
          ...
          return (
            <div 
              key={p.id} 
              onClick={() => navigate(`/product/${p.id}`)} // 6. Make cards clickable
              className="rec-card bg-white border border-stone-200 rounded-2xl overflow-hidden cursor-pointer"
            ...
            >
              <div className="relative overflow-hidden aspect-square bg-stone-50">
                ...
                <button 
                  ...
                  className="absolute top-2 right-2 w-7 h-7 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm border border-stone-100 hover:scale-110 transition-transform"
                >
              ...
              <div className="p-3">
                ...
                <button 
                  onClick={(e) => {
                    e.stopPropagation();
                    navigate("/cart"); // Go directly to cart when adding
                  }}
                  className="w-full mt-2.5 py-2 text-xs font-bold text-emerald-700 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 rounded-xl transition-colors"
                >
                  Add to Cart
                </button>
              </div>
