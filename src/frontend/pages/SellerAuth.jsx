...
export default function SellerAuth() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!isLogin && (!formData.terms1 || !formData.terms2)) {
      alert("Please agree to all terms to submit your application.");
      return;
    }
    setIsLoading(true);
    setTimeout(() => {
      setIsLoading(false);
      if (!isLogin) setCurrentStep(6); 
              ...
              <div className="w-full shrink-0">
                {isLogin ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
                    <InputField label="Email Address" icon={Mail} name="email" type="email" placeholder="store@example.com" required />
