...
export default function Dashboard() {
  ...
  const recentOrders = [
    { id: "ORD-199", items: "2 Items", amount: "₹1,250", status: "Pending Acceptance", time: "5 mins ago" },
    { id: "ORD-198", items: "1 Item", amount: "₹850", status: "Ready to Pack", time: "12 mins ago" },
    { id: "ORD-197", items: "4 Items", amount: "₹3,100", status: "Waiting for Rider", time: "25 mins ago" },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Today's Orders" value="26" />
        <StatCard label="Pending Action" value="4" />
                  ...
                  <tr key={index} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-4 px-2">
                      <span className={`px-2.5 py-1 text-[11px] font-bold rounded-md uppercase tracking-wide ${
                        order.status === 'Pending Acceptance' ? 'bg-amber-50 text-amber-600' :
                        order.status === 'Ready to Pack' ? 'bg-blue-50 text-blue-600' :
