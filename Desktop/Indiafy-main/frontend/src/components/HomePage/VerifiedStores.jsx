import { Star, MapPin, BadgeCheck, ArrowUpRight, Zap } from "lucide-react";
...
function VerifiedStores() {
  ...
  // 🔥 ADDED 'id' TO EACH STORE FOR ROUTING 🔥
  const stores = [
    {
      id: 1,
    ...
    <section className="pt-20 pb-20 md:pt-24 md:pb-24 bg-white border-t border-zinc-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
          {/* FEATURED LARGE CARD (Mobile: Full width, Desktop: 7 cols) */}
          <motion.div
            initial={{ opacity: 0, scale: 0.98 }}
            whileInView={{ opacity: 1, scale: 1 }}
            transition={{ duration: 0.6 }}
            viewport={{ once: true }}
            onClick={() => navigate(`/store/${stores[0].id}`)} // 🔥 NAVIGATE TO SPECIFIC STORE 🔥
          ...
          {/* SIDE LIST CARDS (Mobile: Stacked below, Desktop: 5 cols) */}
          <div className="lg:col-span-5 flex flex-col gap-4 md:gap-6">
            {stores.slice(1).map((store, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                onClick={() => navigate(`/store/${store.id}`)} // 🔥 NAVIGATE TO SPECIFIC STORE 🔥
            ...
            {/* 🔥 NAVIGATION TO ALL SELLERS PAGE 🔥 */}
            <div
              onClick={() => navigate("/local-sellers")}
