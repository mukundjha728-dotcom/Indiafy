...
import { Mail, Lock, User, Eye, EyeOff, Loader2, ArrowRight, ShieldCheck, Sparkles, Github, Chrome as Google } from "lucide-react";
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-[#f0f2f5] p-4 sm:p-8 selection:bg-indigo-100 font-sans">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute -bottom-[10%] -right-[10%] w-[50%] h-[50%] bg-blue-500/10 rounded-full blur-[120px] animate-pulse" style={{ animationDelay: '3s' }} />
      </div>
      ...
      <div className="relative w-full max-w-[850px] bg-white rounded-[2.5rem] shadow-[0_32px_100px_-20px_rgba(0,0,0,0.15)] overflow-hidden flex min-h-[520px]">
        ...
        {/* --- LOGIN SECTION --- */}
        <div className={`form-side w-[55%] p-10 sm:p-14 flex flex-col justify-center transition-all duration-500 ${isLogin ? 'opacity-100 z-1' : 'opacity-0 z-0 pointer-events-none'}`} style={{ marginLeft: 'auto' }}>
            <div className="w-full space-y-8">
                <div className="space-y-2 text-center sm:text-left">
                    <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Welcome Back</h2>
                    <p className="text-slate-500 font-medium text-sm">Please enter your details to login.</p>
                </div>
                ...
                <form onSubmit={handleLoginSubmit(onLogin)} className="space-y-5">
                    <div className="space-y-1.5">
                        ...
                        {loginErrors.email && <p className="text-[10px] text-red-500 font-bold pl-1 uppercase tracking-wider">{loginErrors.email.message}</p>}
                    </div>

                    <div className="space-y-1.5">
                        <div className="flex justify-between items-center px-1">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Password</label>
                            <button type="button" className="text-[10px] font-bold text-indigo-600 hover:text-indigo-700 transition-colors uppercase tracking-wider">Forgot?</button>
                        </div>
                        <div className="relative group">
                ...
                <div className="text-center">
                    <p className="text-xs font-medium text-slate-500">
                        ...
                        <button onClick={handleToggle} className="text-indigo-600 font-bold hover:underline underline-offset-4">Sign Up</button>
                    </p>
                </div>
            </div>
        </div>
        ...
        {/* --- SIGNUP SECTION --- */}
        <div className={`form-side w-[55%] p-10 sm:p-14 flex flex-col justify-center transition-all duration-500 ${!isLogin ? 'opacity-100 z-1' : 'opacity-0 z-0 pointer-events-none'}`}>
            <div className="w-full space-y-8">
                <div className="space-y-2 text-center sm:text-left">
                    <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Create Account</h2>
                    <p className="text-slate-500 font-medium text-sm">Join Indiafy for local shopping magic.</p>
                </div>
                ...
                <form onSubmit={handleSignupSubmit(onSignup)} className="space-y-4">
                    <div className="space-y-1.5">
                        ...
                        {signupErrors.firstName && <p className="text-[10px] text-red-500 font-bold pl-1 uppercase tracking-wider">{signupErrors.firstName.message}</p>}
                    </div>
                    ...
                    <div className="space-y-1.5">
                        ...
                        {signupErrors.email && <p className="text-[10px] text-red-500 font-bold pl-1 uppercase tracking-wider">{signupErrors.email.message}</p>}
                    </div>
                    ...
                    <div className="space-y-1.5">
                        ...
                        {signupErrors.password && <p className="text-[10px] text-red-500 font-bold pl-1 uppercase tracking-wider">{signupErrors.password.message}</p>}
                    </div>
                ...
                <div className="text-center">
                    <p className="text-xs font-medium text-slate-500">
                        Already joined?{" "}
                        <button onClick={handleToggle} className="text-indigo-600 font-bold hover:underline underline-offset-4">Login</button>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div ref={bpRef} className="black-panel absolute inset-0 bg-[#0f172a] z-10 pointer-events-none flex flex-col justify-center p-12 overflow-hidden shadow-2xl">
            ...
            <div ref={wbRef} className="relative z-20 space-y-8 transition-opacity duration-300">
                ...
                {isLogin ? (
                    <div className="space-y-6">
                        <h1 className="text-4xl font-bold text-white leading-tight">Revisit your <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-blue-400">journey.</span></h1>
                        <p className="text-slate-400 text-sm font-medium leading-relaxed max-w-[240px]">Access your orders, wishlist, and lightning-fast local deliveries.</p>
                    </div>
                ) : (
                    <div className="space-y-6">
                        <h1 className="text-4xl font-bold text-white leading-tight">Shop your <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-400">locality.</span></h1>
                        <p className="text-slate-400 text-sm font-medium leading-relaxed max-w-[240px]">Join thousands of locals getting everything they need in minutes.</p>
                    </div>
                )}
                ...
                <div className="pt-6 border-t border-white/10 flex items-center gap-4">
                    <div className="flex -space-x-2">
                        {[1, 2, 3].map(i => (
                            <img key={i} src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${i+10}`} className="w-8 h-8 rounded-full border-2 border-[#0f172a]" alt="avatar" />
                        ))}
                    </div>
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">2k+ Active Locals</span>
                </div>
            </div>
        </div>
